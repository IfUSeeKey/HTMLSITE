...
const correspond = {
    "Объект": "structure",
    "Гид": "guide",
    "Свободно мест": ["vacantFrom", "vacantTo"],
    "Цена за билет, руб": ["priceFrom", "priceTo"],
    "Время начала": ['timeFrom', 'timeTo'],
    "День проведения": ['dayFrom', 'dayTo']
}
    ...
    let tableFilter = data.filter(item => {
        ...
        Object.entries(item).map(([key, val]) => {
            ...
            if (typeof val == 'number') {
                const inBetween = (val, a, b) => a <= val && val <= b;
                result &&= inBetween(val, datafilter[correspond[key][0]], datafilter[correspond[key][1]]);
            }
        });
        return result;
    });
...
// Самостоятельно задание в Задание 2
